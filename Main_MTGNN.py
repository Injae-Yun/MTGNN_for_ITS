...
def run_pipeline(mode="train",version='v1.0.0',
...
# 데이터 전처리 수행
    ...
    if course[2] == 0 :
        Trainer.genmap(link_ids,sorted_clusters,target,path_config,logger,cluster_id= cluster_ids)#cluster_id
    else:
        logger.info(f"✅ map 형성 skip")
    if mode == "train":
        # make map for MTGNN
        training_MTGNN(Config_path, output_dir_model, sorted_clusters,target, logger, cluster_id=cluster_ids) 

    elif mode == "test":
        #이때 train이 아니라면, 각 데이터셋을 test용도로만 정리
        tic=time.time()
        Predict_result=Make_Test_results(output_dir_model,sorted_clusters,logger,cluster_id=cluster_ids) #cluster_id=None
        toc=time.time()
