...
def main(mode="train",version='v1.0.0',
         config_name='Livinglab_test.yaml',target='Livinglab',
...
# 데이터 전처리 수행
    ...
    if mode == "train":
        ...
        training_MTGNN(Config_path, output_dir_model, sorted_clusters,target, logger, cluster_id=0 )
...
if __name__ == "__main__":
    ...
    main(mode="train",version='v1.0.0',config_name=config_name,course=course,db_name=db_name) #v0 위치에 모델 저장
    # config_name = "Livinglab_test.yaml"
    # target= 'Livinglab'
    # main(mode="train",version='v0.0.0',config_name=config_name,target=target) #v0 위치에 모델 저장
"""
    config_name = "HSprediction.yaml"
    target= 'Livinglab'
    course= [0, 0, 0, 0 ] # 1 = skip
    main(mode="test",version='v0.0.0',config_name=config_name,target=target,course=course) #v0 위치에 모델 저장
    """